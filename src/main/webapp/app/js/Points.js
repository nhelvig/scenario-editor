// Generated by CoffeeScript 1.3.3
(function() {
  var __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

  window.sirius.Points = (function(_super) {
    var $a;

    __extends(Points, _super);

    /* $a = alias for sirius namespace
    */


    function Points() {
      return Points.__super__.constructor.apply(this, arguments);
    }

    $a = window.sirius;

    Points.from_xml1 = function(xml, object_with_id) {
      var deferred, fn, obj, _i, _len;
      deferred = [];
      obj = this.from_xml2(xml, deferred, object_with_id);
      for (_i = 0, _len = deferred.length; _i < _len; _i++) {
        fn = deferred[_i];
        fn();
      }
      return obj;
    };

    Points.from_xml2 = function(xml, deferred, object_with_id) {
      var obj;
      if (!(xml != null) || xml.length === 0) {
        return null;
      }
      obj = new window.sirius.Points();
      obj.set('text', xml.text());
      if (obj.resolve_references) {
        obj.resolve_references(deferred, object_with_id);
      }
      return obj;
    };

    Points.prototype.to_xml = function(doc) {
      var xml;
      xml = doc.createElement('Points');
      if (this.encode_references) {
        this.encode_references();
      }
      xml.appendChild(doc.createTextNode($a.ArrayText.emit(this.get('text') || [])));
      return xml;
    };

    Points.prototype.deep_copy = function() {
      return Points.from_xml1(this.to_xml(), {});
    };

    Points.prototype.inspect = function(depth, indent, orig_depth) {
      if (depth == null) {
        depth = 1;
      }
      if (indent == null) {
        indent = false;
      }
      if (orig_depth == null) {
        orig_depth = -1;
      }
      return null;
    };

    Points.prototype.make_tree = function() {
      return null;
    };

    return Points;

  })(Backbone.Model);

}).call(this);
